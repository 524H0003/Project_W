...
export const useRecord = (
	...
	const filteredRecord = initialRecord
		? filterRecordParams(initialRecord, options)
		: null;
	...
	const setFilteredRecord: Dispatch<SetStateAction<RecordJSON>> = useCallback(
		(value) => {
	...
	const handleChange = useCallback(
		(
			...
			if (isEntireRecordGiven(propertyOrRecord, value)) {
				...
				setRecord(
					updateRecord(propertyOrRecord as string, value, incomingRecord),
				);
			...
			promise
				.then((response) => {
					...
					if (submitOptions?.updateOnSave !== false) {
						setFilteredRecord((prev) =>
							mergeRecordResponse(prev, response.data),
						);
					}
					setProgress(0);
					setLoading(false);
