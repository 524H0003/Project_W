...
const getActionElementCss = (...args: (string | number)[]) => args.join('-'),
	Edit: FC<ActionProps> = (props) => {
		const { record: initialRecord, resource, action } = props,
			{
				...
				initialRecord,
				resource.id,
				async () => await getCsrfToken('/admin'),
			),
			...
			footerTag = getActionElementCss(
				resource.id,
				action.name,
				'drawer-footer',
			),
			buttonTag = getActionElementCss(
				resource.id,
				action.name,
				'drawer-submit',
			);
